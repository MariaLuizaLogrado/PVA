...
class Canny:
    def __init__(self, mouth, mouth_x, mouth_y,
        ...
        self.mid_point_le = mid_point_le
        self.mid_point_re = mid_point_re


    def edge_detection(self, image):
        return cv2.Canny(image, 150, 200)
    
    def coordenates(self, edge, x, y, mid_point_car):
        coord = np.argwhere(edge == 255)
        sum = np.array([y,x])
        coord_sum = coord + sum

        # coord_mid = coord_sum - self.mid_point
        coord_mid = coord_sum - mid_point_car
    ...
    def compute_all_edges(self):
        ...
        self.mouth_dict = self.coordenates(self.mouth_edge, self.mouth_x, self.mouth_y, self.mid_point_mouth)
        self.nose_dict = self.coordenates(self.nose_edge, self.nose_x, self.nose_y, self.mid_point_nose)
        self.left_eye_dict = self.coordenates(self.left_eye_edge, self.left_eye_x, self.left_eye_y, self.mid_point_le)
        self.right_eye_dict = self.coordenates(self.right_eye_edge, self.right_eye_x, self.right_eye_y, self.mid_point_re)
